...
import { ArrowRight, BookOpen, GraduationCap, PlayCircle, ShieldCheck, Target, Users } from "lucide-react";
  ...
  if (isAuthenticated && user) {
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 py-8">
        <div className="overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 p-8 text-white shadow-sm">
          <p className="text-indigo-100">Xin chào,</p>
          <h1 className="mt-1 text-3xl font-bold">{user.fullName || user.username}</h1>
          <p className="mt-3 max-w-2xl text-indigo-100">Tiếp tục lộ trình học tập, theo dõi tiến độ và quay lại các khóa học đang mở.</p>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <StatCard label="Khóa đã đăng ký" value={myCourses.length} icon={<BookOpen size={20} />} />
          <StatCard label="Tiến độ trung bình" value={`${avg}%`} icon={<Target size={20} />} tone="emerald" />
          <StatCard label="Khóa đang học" value={myCourses.filter((c) => (c.progress || 0) < 100).length} icon={<PlayCircle size={20} />} tone="sky" />
        </div>
        ...
        {myCourses.length ? <div className="mt-4 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{myCourses.map((course) => <CourseCard key={course.id} course={course} />)}</div> : (
          <div className="mt-4"><EmptyState title="Chưa có khóa học" description="Đăng ký một khóa học để bắt đầu theo dõi tiến độ." /></div>
        )}
      </div>
    );
  }
    ...
    <div>
      <section className="bg-slate-950 text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            <span className="rounded-full bg-white/10 px-4 py-2 text-sm text-indigo-100">Hệ thống quản lý khóa học online</span>
            ...
            <p className="mt-5 max-w-2xl text-lg text-slate-300">EduFlow hỗ trợ demo đầy đủ phân quyền theo vai trò, danh sách khóa học, học bài và quản lý vận hành bằng mock data.</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        <div className="grid gap-4 md:grid-cols-4">
          {[["Lộ trình rõ ràng", BookOpen], ["Quản lý học viên", Users], ["Giảng viên chủ động", GraduationCap], ["Phân quyền theo vai trò", ShieldCheck]].map(([label, Icon]) => (
            <div key={String(label)} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md"><Icon className="mb-3 text-indigo-600" /><strong>{String(label)}</strong></div>
          ))}
        </div>
